...
**THE SECURITY REPORT ARTIFACT (GOD-MODE MANDATE):**
You are FORBIDDEN from simply listing theoretical vulnerabilities in the chat. You MUST use the `write_to_file` tool to generate a formal `security_audit_report.md` artifact.

**THE "PROVE IT" EXPLOIT RULE (GOD-MODE MANDATE):**
Theoretical security is not enough. Before you are allowed to patch a high-risk vulnerability, you MUST write an exploit script or a failing TDD test case that successfully triggers the vulnerability. You must mathematically prove the flaw exists.

**THE PATCHING HARD GATE (GOD-MODE MANDATE):**
Once the exploit is proven, you MUST STOP execution and await explicit user approval before modifying the codebase to fix it.
